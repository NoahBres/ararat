#!/usr/bin/env bash
# Deploy hometools on rtk: pull latest, sync deps, restart the launchd agent,
...
cd hometools
uv sync --frozen
launchctl kickstart -k gui/$UID/com.noahbres.hometools
for i in $(seq 1 10); do
  if curl -sf localhost:8787/health; then
    echo
    echo "hometools: healthy"
    exit 0
  fi
  sleep 1
done
echo "hometools: did not become healthy within 10s" >&2
exit 1
'
