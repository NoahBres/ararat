...
A request is authorized if ANY of:
  ...
  3. Path starts with `/{HOMETOOLS_MCP_SECRET}/` (the MCP capability URL).
...
from hometools.config import Settings

logger = logging.getLogger("hometools.auth")
...
def _verify_bearer(header_value: str, settings: Settings) -> bool:
    if not settings.hometools_bearer_token:
        return False
    if not header_value.startswith("Bearer "):
        return False
    presented = header_value[len("Bearer ") :]
    return hmac.compare_digest(presented, settings.hometools_bearer_token)
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        mcp_secret = settings.hometools_mcp_secret
