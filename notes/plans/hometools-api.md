...
| Repo location | `hometools/` subfolder of this repo (`~/Developer/ararat/hometools`). rtk already pulls this repo on restart. |
...
| Secrets | Never in the repo. Runtime secrets live in `~/.config/hometools/env` on rtk (`chmod 600`), copied from 1Password. |
...
Each tool module in `hometools/tools/` exposes plain functions with type hints and docstrings.
A registry decorator registers each function once, and the app generates **both** a FastAPI
route and an MCP tool from it:

```python
# hometools/registry.py (sketch)
...
- Write tools must (a) be no-ops with a clear error when their enable flag is off, (b) log to `~/.local/state/hometools/audit.log` (timestamp, tool, caller principal, args) before executing.

### Auth middleware (`hometools/auth.py`)
...
2. `Authorization: Bearer <HOMETOOLS_BEARER_TOKEN>` (constant-time compare).
3. Path starts with `/{HOMETOOLS_MCP_SECRET}/` (the MCP capability URL). Only the MCP mount lives under that prefix.
...
2. **Cloudflare Access for `api.noahbres.com`.** Zero Trust → Access → Applications → Add → Self-hosted; domain `api.noahbres.com`. Policy: action **Service Auth**, include **Service Token** (create one: Access → Service Auth → Service Tokens → "hometools-api"; save Client ID + Client Secret to 1Password as item `hometools api service token`). Copy the application's **AUD tag** (Overview tab) and the team domain (`<team>.cloudflareaccess.com`).
...
4. **Secrets file on rtk** at `~/.config/hometools/env`, `chmod 600`:
   ```
   ...
   ```
   Also save the bearer token and MCP secret to 1Password (`hometools`).
...
All work happens in `hometools/` in this repo. Develop on rnn, deploy by pushing + pulling on rtk.

1. **Scaffold** with `uv init --package hometools` (Python `>=3.12`). Deps: `fastapi`, `uvicorn[standard]`, `fastmcp`, `pydantic`, `pyjwt[crypto]`, `httpx`, `things.py` (PyPI name `things.py`, import `things`). Dev deps: `pytest`, `pytest-asyncio`, `httpx`, `ruff`.
...
2. **Files:**
   - `hometools/pyproject.toml`
   - `hometools/hometools/__init__.py` (`__version__`)
   - `hometools/hometools/config.py` — pydantic-settings, reads env + `~/.config/hometools/env`.
   - `hometools/hometools/registry.py` — as §2.
   - `hometools/hometools/auth.py` — as §2.
   - `hometools/hometools/app.py` — builds FastAPI app, mounts MCP, exposes `main()` for uvicorn (`hometools serve --port 8787`).
   - `hometools/hometools/tools/system.py` — `system.ping`, `system.version`, `system.echo`.
   - `hometools/hometools/tools/things.py` — read tools (below).
   - `hometools/tests/` — unit tests with a fake registry tool and auth tests (bearer ok / missing / wrong; secret path ok; fake JWT rejected).
   - `hometools/README.md` — how to run locally, env vars, how to add a tool (copy from §2).
...
4. **Local run on rnn**: `cd hometools && uv run hometools serve` → `curl -H "Authorization: Bearer $T" localhost:8787/v1/things/list -d '{"view":"today"}'`. (rnn has Things too, so this is testable locally.)
5. **launchd agent on rtk** in `nixos-config/hosts/rtk/home.nix`, modelled on `things-today-tracker`:
   - Label `com.noahbres.hometools`; ProgramArguments = a `pkgs.writeShellScript` that does `cd ~/Developer/ararat/hometools && exec uv run --frozen hometools serve --host 127.0.0.1 --port 8787`; `EnvironmentVariables.PATH` same as the other agents; `KeepAlive.SuccessfulExit=false`; `RunAtLoad=true`; logs to `/tmp/hometools.log` / `/tmp/hometools-error.log`.
   - The start script should `set -a; source ~/.config/hometools/env; set +a` before exec.
   ...
   - Add shell aliases `restart-hometools` / `hometools-log` like the ararat ones.
   - Apply with `just switch-rtk` from `nixos-config/` (see `justfile`). Verify `launchctl list | grep hometools` and `curl localhost:8787/health` on rtk.
6. **Edge verification** from rnn:
   ...
   ```
7. **Docs**: add `hometools` to the "Available Capabilities" section of `CLAUDE.md` and a short section in `notes/NOTES.md` (hostnames, where secrets live, how to restart).

Acceptance: REST works through Cloudflare with the service token; MCP endpoint at `https://mcp.noahbres.com/<secret>/mcp` responds to `initialize` (test with `npx @modelcontextprotocol/inspector` or `claude mcp add --transport http hometools https://mcp.noahbres.com/<secret>/mcp` then `claude mcp list`).
...
`uv python find 3.12` inside `hometools/`). System Settings → Privacy & Security → Full Disk Access →
`+` → press ⌘⇧G and paste that path → enable. Then `restart-hometools`. If uv later upgrades the
...
- Never commit anything from `~/.config/hometools/env`, `.env`, or 1Password. `hometools/.env*` must be in `.gitignore`.
- Deploy loop: push from rnn → on rtk `cd ~/Developer/ararat && git pull --ff-only && cd hometools && uv sync --frozen && restart-hometools` (put this in `hometools/deploy.sh` and mention it in README). The nix agent only needs re-applying when `home.nix` changes.
...
1. Name: the service is called `hometools` in this plan. Rename freely before Phase 1 starts.
